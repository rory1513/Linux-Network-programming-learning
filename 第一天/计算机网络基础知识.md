...
网络层最核心的协议是IP协议(Internet Protocol,因特网协议)。IP协议根据数据包的目的IP地址来决定如何投递它。如果数据包不能直接发送给目标主机，那么IP协议就为它寻找一个合适的下一跳(next hop)路由器，并将数据包交付给该路由器来转发。多次重复这一过程，数据包最终到达目标主机，或者由于发送失败而被丢弃。可见，**IP协议使用逐跳(hop by hop)的方式**确定通信路径。
...
SCTP协议(Stream Control Transmission Protocol,流控制传输协议)是传输层的一个新协议，它提供的是面向连接的、可靠的数据流服务。SCTP协议与TCP协议类似，提供超时重传机制，以保证传输的可靠性。SCTP协议还提供流量控制和拥塞控制机制，以防止发送端发送速率过大导致网络拥塞。
